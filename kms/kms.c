...
// Structure to store the response of cURL
struct MemoryStruct {
    char *memory;
    size_t size;
};

// Callback to manage the response of cURL
...
// Function to commit requests HTTPs
...
// Function to extract the KEY and ID_KEY from the JSON
void extract_key_and_id(const char *json_str, char *key_buffer, size_t key_buffer_len, char *key_id_buffer, size_t key_id_buffer_len) {
    ...
    if (!root) {
        fprintf(stderr, "Error in JSON in line %d: %s\n", error.line, error.text);
        return;
    }
...
// Function to extract key and ID from a JSON and decode Base64 key
